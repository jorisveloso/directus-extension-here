...
export default defineHook(({ init }, { services, database, getSchema }) => {
  ...
  init("routes.custom.after", async () => {
    ...
    if (directusState.collections) {
      ...
      const collections = Array.isArray(directusState.collections)
      ...
      const collections_with_group = collections.filter(
        (col) => col.meta.group !== null
      );
      const collections_without_group = collections.filter(
        (col) => col.meta.group == null
      );
      const sorted_collections = [
        ...collections_without_group,
        ...collections_with_group,
      ];
    ...
    if (directusState.relations) {
      const relationsService = new RelationsService({
        knex: database,
        schema: await getSchema({ database: database, bypassCache: true }),
      });
      const relations = Array.isArray(directusState.relations)
    ...
    const paisService: ItemsService = new ItemsService("pais", {
      knex: database,
      schema: await getSchema(),
    });
